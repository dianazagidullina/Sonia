...
def get_sample(seed, path):
    decoder = torch.load(path).to(device)
    input_rand = []
    for i in range(0, hidden_size):
        input_rand.append(seed)
    input_rand = np.reshape(input_rand, (1, 1, hidden_size))
    new_indexes = evaluate(input_rand, decoder, max_length=150, most_prob=15)
    mid_new = midi_from_indexes(new_indexes[10:], max_dur_note, amount_dur, each_dur)
    mid_new.save('new_song1.mid')
    print('OK')
    # using the default sound font in 44100 Hz sample rate
    fs = FluidSynth('/Users/diana/Desktop/TimGM6mb.sf2')
    fs.midi_to_audio('new_song1.mid', 'output.wav')


get_sample(3, "/Users/diana/Documents/Project/models_Sonya/Last_6___4_10_0.2")